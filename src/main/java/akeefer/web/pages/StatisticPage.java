...
public class StatisticPage extends AbstractAuthenticatedBasePage {
    ...
    public StatisticPage(final PageParameters parameters) {
        ...
        Form<StatisticPage> form = new Form<StatisticPage>("form", new CompoundPropertyModel<StatisticPage>(this)) {
            @Override
        ...
        form.add(new AjaxFallbackButton("submit", form) {
            ...
            protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
                if (null != target) {
                    ...
                    target.add(chartPie);
                    target.add(getPageFeedbackPanel());
                }
            }
        });
    }
        ...
        for (Map.Entry<AktivitaetsTyp, BigDecimal> dataEntry : data.entrySet()) {
            pointSeries.addPoint(new Point(dataEntry.getKey().toVergangenheit(), dataEntry.getValue())
                    .setColor(new RadialGradient()
                            ...
                            .addStop(1, new HighchartsColor(i)
                                    .brighten(-0.3f))));
            i++;
        }
